				...
				$.ajax({
					...
					success : function (data) {
						...
						$.each(data, function (i, n) {
							html += '<tr>';
							html += '<td><input name="select" value="'+ n.id +'" type="checkbox"/></td>';
							html += '<td>'+ n.name +'</td>';
						...
						$("#activitySearch").html(html);
					}
				})


				// 展现完列表后，需要将模态窗口默认的回车行为禁用
				return false;
			}
		})

		// 为关联市场活动的模态窗口的关联（即保存）按钮绑定事件
				...
				$.ajax({
					...
					success : function (data) {
						if (data.success) {
							...
							// 刷新列表
							showActivityList();
							// 清空搜索框内容，复选框中的选择，以及activitySearch中的内容
							$("#aname").val("");
							$("input[name=select]:checked").val("");

							$("#activitySearch")[0].reset();
							// 关闭模态窗口
