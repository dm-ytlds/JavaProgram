# 项目描述
该项目为贸易行业的客户管理系统，主要针对的是企业用户，单方面的对客户做出的一些管理，例如：售前、售中、售后。
前台包括的模块有：工作台、目前整个企业的客户关系动态、审批、客户公海、市场活动、线索、客户、联系人、交易、
               售后回访、统计图表、报表、销售订单、发货单、跟进、产品、报价等；
后台包括的模块有：个人设置、部门维护、权限管理、数据字典等。
...
# 项目职责
主要负责用户登录模块、市场活动模块（模态窗口的创建、修改、查询等操作）、线索模块（线索的创建值数据字典的使用、
关联到市场活动列表的设计、添加市场交易信息等）、交易模块（在交易信息页面展示详细的交易信息，获取交易历史列表，
动态展示交易阶段的内容以及图标，并且可以点击交易图标，更改交易的阶段等操作）以及统计图表模块
（ECharts统计图的使用，主要用于交易阶段的图表展示）
...
### 验证登录模块
 ...
 #### 登录成功
    ...
    登录成功后，返回user对象，并保存到session域对象中，跳转到主页面。
 ...
 #### 知识点解释
    ...
    2. cache（缓存机制）解决数据字典存储问题：避免每次都从数据库取数据，省去了连接数据库的操作，节省时间，节省内存
        ...
        (2) application（全局作用域，也叫做上下文作用域）：在服务器启动阶段，将数据保存在服务器的缓存中，服务器启动多久，数据就存在多久。
         ...
         将数据从服务器缓存中取出的方法：application.getAttribute();
    3. 数据字典是什么
    ...
    8. JUnit介绍
        JUnit是测试工具包，采用的是多线程方式，可以批量测试多个方法。